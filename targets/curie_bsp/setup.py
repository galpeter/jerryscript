...
def build_soft_links(project_path, jerry_path):
    ...
    links = [
        { # arc
            'src': 'targets/curie_bsp/jerry_app/arc',
            'dst': 'arc'
        },
        { # include
            'src': 'targets/curie_bsp/jerry_app/include',
            'dst': 'include'
        },
        { # quark
            'src': 'targets/curie_bsp/jerry_app/quark',
            'dst': 'quark'
        },
        { # quark/jerryscript
            'src': jerry_path,
            'dst': os.path.join('quark', 'jerryscript')
        }
    ]

    for link in links:
        src = os.path.join(jerry_path, link['src'])
        dst = os.path.join(project_path, link['dst'])
...
def find_sources(root_dir, sub_dir):
    """
    Find .c and .S file inside the @root_dir/@sub_dir directory.
...
def build_jerry_data(jerry_path):
    ...
    jerry_sources = []
    jerry_dirs = set()
    for sub_dir in ['jerry-core', 'jerry-libm', 'targets/curie_bsp/source']:
...
def build_obj_y(source_list):
    ...
    """
    return '\n'.join(['obj-y += %s.o' % os.path.splitext(fname)[0] for fname in source_list])


def build_subdir_cflags_y(cflags_list):
    """
    Build subdir-cflags-y additions from the @cflags_list.
    Note: the input sources should have their file extensions.
    """
    return '\n'.join(['cflags-y += %s' % cflag for cflag in cflags_list])


def build_mkdir(dir_list):
    """ Build mkdir calls for each dir in the @dir_list. """
    return '\n'.join(['\t$(AT)mkdir -p $(OUT_SRC)/%s' % path for path in dir_list])
...
def create_quark_kbuild(project_path, jerry_path):
    ...
    quark_include_paths = [
        'include',
        'jerryscript',
        'jerryscript/jerry-libm/include',
        'jerryscript/targets/curie_bsp/include'
    ] + list(jerry_data['dirs'])

    quark_includes = [
        '-Wno-error',
    ] + ['-I$(PROJECT_PATH)/quark/%s' % dir for dir in quark_include_paths]

    quark_cflags = build_subdir_cflags_y(jerry_defines + quark_includes)
